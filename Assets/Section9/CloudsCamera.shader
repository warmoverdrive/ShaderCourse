Shader "Holistic/Volumetrics/CloudsCamera"
{
	Properties
	{
		_MainTex("", 2D) = "white" {}
	...
	{   
		...
		Pass
		{
			...
			{
				fixed3 lighting = _LightColor1.rgb * 1.3 + 0.5 * _LightColor2.rgb * diffuse;
				fixed3 colrgb = lerp( _BrightColor.rgb, _DarkColor.rgb, density);
				fixed4 col = fixed4(colrgb.r, colrgb.g, colrgb.b, density);
				col.rgb *= lighting;
				col.rgb = lerp(col.rgb, bgcol, 1-exp (-0.01*t*t));
				col.a *= 0.5;
				col.rgb *= col.a;
